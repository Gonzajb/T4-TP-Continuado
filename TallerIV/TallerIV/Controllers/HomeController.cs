            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(cs))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(store1, connection))
                    {
                        ...
                        cmd.ExecuteNonQuery();
                        string store3 = @"CREATE PROCEDURE EstadisticaPorcentajeAviso2 (@avisoId INT) AS BEGIN
...
ORDER BY Temp2.PorcentajeDePuntos END";                        
                        ...
                        cmd.ExecuteNonQuery();
                        string store4 = @"CREATE PROCEDURE EstadisticaPorcentajeAviso (@avisoId INT) AS BEGIN
SELECT PorcentajeDePuntos,
       Cantidad,
       CAST (cast(Cantidad AS float)/ CAST (TotalUsuarios AS float) AS decimal (10,1))*100 AS PorcentajeDeUsuarios,
            MaximoAviso
